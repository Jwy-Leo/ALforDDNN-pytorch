...
class NTHUSCDet_Dataset(Dataset):
    def __init__(self, dataset_image_folder_path, dataset_label_json_path,\
        ...
        for _folder in self.video_names:
            sub_video_img_folder_path = os.path.join(mf, _folder, "img1")
            mf2, _, _files2 = next(iter(os.walk(sub_video_img_folder_path)))
            _files2_tuple = [("".join(item), int(item[0])) for item in map(os.path.splitext, _files2)]
        ...
        self.TENSOR_IN_MEMORY = False #True
    ...
    def get_annotation(self, index):
        video_name, frame_name = self.mapping_video(index)
        bboxes, classes = self._load_gt(video_name, frame_name)
